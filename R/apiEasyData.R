...
##' @param x either a charachter string containing the whole url for the request or a named list containing the parameter names and values (see details)
...
apiEasyData <- 
  function(x, base.url, shortnames=TRUE, ...) {
    ...
    if(is.list(x)) { # parameter/values as list? proceed with apiDatalight...
      ...
      # cosmetics
      if (shortnames==TRUE) {data <- lapply(data, onlyLeafnames)}
      if (length(data)==1 & is.data.frame(data[[1]])) {data <- data[[1]]}
