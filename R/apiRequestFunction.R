...
##' @param x a two-column data frame or a named list containing the parameter names and values (see details).
...
function(x, base.url, multiparam=NULL, key.param=NA, key.object=NA, vectorizeIt=FALSE){
    ...
    if (!is.data.frame(x) & is.list(x) & !is.null(multiparam) ) { stop("multiparam can only be used in combination with x as a data frame.\nx is not a data frame.")}
    if (!is.data.frame(x) & is.list(x) & vectorizeIt==TRUE ) { stop("vectorizeIt=TRUE can only be used in combination with x as a data frame.\nx is not a data frame.")}
    ...
    e.object <- expression(apirequest <- new("apirequest", URL=url, request.arguments=rarg, nodefault.parameters=ndrequest, server=server ))
    ...
    if(is.character(multiparam)) { # add multiparam to 
      ...
      multistring <- paste0("for (i in 1:length(", multiparam, ")) { mparams <- paste0(mparams, '&", multiparam,"=', ", multiparam, "[i])}")
      ...
      e <- c(e.args, e.params, e.multi1, e.multi2, e.multi3, e.multi4, e.key, e.url, e.paramdf, e.namesdf, e.server, e.object, e.return)
